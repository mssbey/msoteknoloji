  ...
  return (
    <div className="space-y-5 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-white">Kampanyalar</h1>
          <p className="text-sm text-white/40 mt-0.5">Sezon, flash sale ve özel gün kampanyalarını yönetin</p>
        </div>
        ...
        {CAMPAIGNS.map((c, i) => (
          <div key={i} className="glass-card p-5">
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 shadow-lg shadow-purple-500/20 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-blue-400 font-semibold">{c.discount} · {c.products} ürün</p>
                ...
                <div className="flex items-center gap-4 mt-3 text-xs text-white/50 flex-wrap">
                  ...
                  <span className="flex items-center gap-1.5"><TrendingUp className="h-3 w-3 text-green-400" />CTR %{((c.clicks / Math.max(parseFloat(c.impressions.replace('K', '')) * 1000, 1)) * 100).toFixed(1)}</span>
                </div>
              </div>
